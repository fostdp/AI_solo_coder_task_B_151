...
public class ParallelCoordinationOptimizer {
    ...
    public ParallelOptimizationDTO optimizeParallel(List<Integer> deviceIds,
        ...
        double totalCap = Arrays.stream(capacities).sum();
        ...
        for (iter = 0; iter < MAX_ITERATIONS; iter++) {
            ...
            double lambdaFlow = totalFlow > 0 ? targetFlow / totalFlow : 1.0;
            double objFlow = Math.pow(totalFlow - targetFlow, 2);
            ...
            tp.setTotalFlow(round(totalFlow));
            ...
            double flowError = targetFlow - totalFlow;
            double[] gradients = new double[N];
            for (int i = 0; i < N; i++) {
                double grad = 0;
                if ("MAX_FLOW".equals(goal)) {
                    grad = -flowSensitivity(devices.get(i), speeds[i], depths[i]);
                } else if ("MIN_POWER".equals(goal)) {
                    grad = powerPerFlow[i] * flowSensitivity(devices.get(i), speeds[i], depths[i]) / 1000.0;
                } else {
                    double flowPart = -0.5 * flowError * flowSensitivity(devices.get(i), speeds[i], depths[i]);
                    double balancePart = 0.3 * (flows[i] - (totalFlow / N)) * flowSensitivity(devices.get(i), speeds[i], depths[i]);
                    double powerPart = 0.2 * powerPerFlow[i] * flowSensitivity(devices.get(i), speeds[i], depths[i]) / 1000.0;
            ...
            if (totalFlow > 0 && totalFlow > targetFlow * 1.15) {
                double scale = targetFlow * 1.1 / totalFlow;
                for (int i = 0; i < N; i++) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            double rM = d.getSprocketRadiusCm() / 100.0;
            ...
            da.setAssignedFlowRatio(round(flow / Math.max(totalPredictedFlow, 1e-9)));
            da.setAssignedFlowLh(round(flow));
        ...
        double meanFlow = totalPredictedFlow / N;
        ...
        dto.setTotalPredictedFlowLh(round(totalPredictedFlow));
    ...
    private double evaluateFlow(WaterwheelDevice d, double speedMs, double depth, double width, double angle) {
        return efficiencyOptimizer.calculateWaterFlow(depth, width, angle, speedMs);
    }
